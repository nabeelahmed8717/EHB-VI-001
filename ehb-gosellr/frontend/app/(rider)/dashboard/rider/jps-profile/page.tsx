...
/**
 * Rider's JPS profile connect page. Mirrors the seller's page at
 * /(seller)/dashboard/jps-profile. The only meaningful differences:
 *   - Uses role=rider for the JPS picker
 *   - "Why does this matter?" copy says "Sellers can only send delivery
 *     requests once you've linked your JPS profile."
 *   - Unlink is blocked while on an active delivery (server-enforced).
 */
export default function RiderJpsProfilePage() {
  const search = useSearchParams();
  const router = useRouter();
  const fromJps = search.get('from') === 'jps';

  const { data: rider, isLoading: riderLoading } = useGetRiderProfileQuery();
  const { data: linked, isLoading: linkedLoading } =
    useGetLinkedRiderJpsProfileQuery(undefined, {
      skip: !rider?.jps_profile_id,
    });

  const [attach, { isLoading: attaching }] = useAttachRiderJpsProfileMutation();
  const [autoAttach, { isLoading: autoAttaching }] = useAutoAttachLatestRiderJpsProfileMutation();
  const [unlink, { isLoading: unlinking }] = useUnlinkRiderJpsProfileMutation();

  const [pickerOpen, setPickerOpen] = useState(false);
  const { data: eligible, isFetching: eligibleLoading } =
    useGetEligibleRiderJpsProfilesQuery(undefined, { skip: !pickerOpen });

  // ── Deep-link return flow ────────────────────────────────────────────────
  const [autoTried, setAutoTried] = useState(false);
  useEffect(() => {
    if (!fromJps || autoTried || riderLoading) return;
    if (rider?.jps_profile_id) return;
    setAutoTried(true);
    (async () => {
      try {
        ...
        toast({
          title: 'Linked!',
          description: 'Your new JPS profile is attached. Sellers can now send you delivery requests.',
        });
        router.replace('/dashboard/rider/jps-profile');
      } catch (err: unknown) {
  ...
  }, [fromJps, autoTried, riderLoading, rider, autoAttach, router]);
  ...
  const createNewUrl = (() => {
    const ret = `${typeof window !== 'undefined' ? window.location.origin : 'http://localhost:4002'}/dashboard/rider/jps-profile?from=jps`;
  ...
  async function handleAttach(profileId: string) {
    try {
      ...
      toast({ title: 'Linked!', description: 'JPS profile attached. You can now receive delivery requests.' });
      setPickerOpen(false);
    } catch (err: unknown) {
      const e = err as { data?: { message?: string } };
      toast({
  ...
  async function handleUnlink() {
    if (!confirm('Unlink this JPS profile? Sellers will no longer be able to send you delivery requests until you relink.')) return;
    try {
      await unlink().unwrap();
      toast({ title: 'Unlinked', description: 'JPS profile detached from this rider account.' });
    } catch (err: unknown) {
      const e = err as { data?: { message?: string } };
      toast({
        title: 'Cannot unlink',
    ...
    <div className="max-w-2xl space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Connect your JPS identity to this rider account so sellers can send you
          verified delivery requests. Until you link an SQ-approved profile, you
          won&apos;t appear in any seller&apos;s Assign Rider modal.
        </p>
      </div>

      {/* ── Linked state ─────────────────────────────────────────────────── */}
      {rider?.jps_profile_id ? (
        <Card>
          <CardHeader>
          ...
          <CardContent className="space-y-4">
            {linkedLoading ? (
                ...
                {linkedSummary.status !== 'approved' && (
                  <div className="flex items-start gap-2 bg-blue-50 border border-blue-100 rounded p-3">
                    ...
                    <p className="text-xs text-blue-700">
                      Your JPS profile is not yet verified. Sellers won&apos;t be able to
                      send delivery requests until JPS marks it approved.
                    </p>
                  </div>
                )}
              </>
              ...
              <p className="text-sm text-muted-foreground">
                Profile metadata unavailable (JPS may be down). Linkage is intact —
                sellers will be able to see you again as soon as JPS responds.
              </p>
            )}
            <div className="flex gap-2 pt-2 border-t border-border">
              <Button
                ...
                asChild
              >
                <a href={`${JPS_FRONTEND_URL}/profiles/${rider.jps_profile_id}`} target="_blank" rel="noreferrer">
              ...
              <Button
                ...
                disabled={unlinking || rider.availability === 'on_delivery'}
                className="text-destructive hover:text-destructive"
                title={
                  rider.availability === 'on_delivery'
                    ? 'Cannot unlink during an active delivery'
                    : undefined
                }
              >
        ...
        <Card className="border-2 border-dashed border-orange-200 bg-orange-50/40">
          ...
          <CardContent className="space-y-3">
            <p className="text-sm text-foreground/80">
              You need a JPS profile (platform <strong>gosellr</strong>, role{' '}
              <strong>rider</strong>) before sellers can send you delivery requests.
        ...
        <Card>
          ...
          <CardContent>
            {eligibleLoading ? (
              ...
              <div className="space-y-2">
                {eligible.map((p: EligibleRiderJpsProfile) => (
                  <div
                    key={p.id}
                    ...
                    <div className="min-w-0">
                      ...
                      {p.already_linked && !p.linked_to_me && (
                        <p className="text-xs text-red-600 mt-1">
                          Already linked to a different GoSellr rider account
                        </p>
                      )}
                    </div>
                    <Button
      ...
      <p className="text-xs text-muted-foreground">
        <Link href="/dashboard/rider" className="hover:underline">← Back to dashboard</Link>
      </p>
    </div>
  );
}
