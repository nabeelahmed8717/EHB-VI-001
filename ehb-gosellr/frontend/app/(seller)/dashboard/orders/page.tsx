...
import { AssignRiderModal } from '@/components/seller/AssignRiderModal';
import { DeliveryRequestStatus } from '@/components/seller/DeliveryRequestStatus';
...
// Statuses where the SELLER has the next manual action (button on card).
// ready_for_delivery is intentionally absent — assignment now goes through
// the DeliveryRequestStatus widget (Assign Rider modal + live status chip).
...
const ACTION_LABELS: Record<string, string> = {
  ...
  ready_for_delivery: 'Mark ready for pickup',
};
...
export default function SellerOrdersPage() {
  ...
  // Live updates: invalidates RTK caches when delivery_request:* fires.
  useOrdersSocket();

  const statuses = [
    'all', 'pending', 'confirmed', 'ready_for_delivery',
    ...
    <div className="space-y-6">
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-pill text-sm font-medium capitalize transition-colors ${
              filter === s
                ...
                : 'bg-card border border-border text-muted-foreground hover:bg-surface-alt hover:text-foreground'
            }`}
          >
            {s.replace(/_/g, ' ')}
          </button>
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {filtered.map((order) => {
            ...
            const showRiderWidget = order.status === 'ready_for_delivery';
            return (
              <div
                key={order.id}
                ...
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span
                        className={`text-xs px-2.5 py-0.5 rounded-pill font-semibold ${STATUS_COLORS[order.status]}`}
                      >
                  ...
                  {showRiderWidget ? (
                    <DeliveryRequestStatus
                      orderId={order.id}
                      onClickAssign={() => setAssignFor(order)}
                    />
      ...
      {assignFor && (
        <AssignRiderModal
          ...
          orderArea={`${assignFor.delivery_address.area}, ${assignFor.delivery_address.city}`}
        />
      )}
    </div>
  );
